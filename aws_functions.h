...
static inline struct AwsCanonicalRequestDetails ngx_aws_auth__make_canonical_request(ngx_pool_t *pool,
																					 const ngx_http_request_t *req,
																					 const ngx_str_t *s3_bucket_name, const ngx_str_t *amz_date, const ngx_str_t *amz_security_token, const ngx_str_t *s3_endpoint)
{
	...
	const struct AwsCanonicalHeaderDetails canon_headers =
		ngx_aws_auth__canonize_headers(pool, req, s3_bucket_name, amz_date, request_body_hash, amz_security_token, s3_endpoint);
															 ...
															 const ngx_str_t *access_key_id, const ngx_str_t *key_scope)
{
	...
	authz->len = access_key_id->len + key_scope->len + signed_header_names->len + signature->len + sizeof(FMT_STRING);
																			 ...
																			 const ngx_str_t *s3_endpoint)
{
	...
	const struct AwsCanonicalRequestDetails canon_request =
		ngx_aws_auth__make_canonical_request(pool, req, s3_bucket_name, date, aws_security_token, s3_endpoint);
...
static inline const ngx_array_t *ngx_aws_auth__sign(ngx_pool_t *pool, ngx_http_request_t *req,
													...
													const ngx_str_t *s3_endpoint)
{
	const struct AwsSignedRequestDetails signature_details = ngx_aws_auth__compute_signature(pool, req, signing_key, key_scope, s3_bucket_name, aws_security_token, s3_endpoint);
